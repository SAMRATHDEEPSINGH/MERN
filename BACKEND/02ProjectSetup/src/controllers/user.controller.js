...
const removeImageFromcloudinary=async (userImgUrl)=>{
    try {
        const urlArray=userImgUrl.split('/');
...
const updateUserAvatar=asyncHandler(async(req,res)=>{
    ...
    if (req.user.avatar.url) {
      await removeImageFromcloudinary(req.user.avatar.url)
    }
   ...
   const user= await User.findByIdAndUpdate(req.user?._id,
    ...
    .status(200)
    .json(
        new ApiResponse(200,user,"Avatar was updated")
    )
})
